        ...
        with self.env.begin(write=False) as txn:
            ...
            if self.opt["dataset"]["filter_on"]:
                ...
                for index in range(self.nSamples):
                    ...
                    if len(label) > self.opt["training"]["batch_max_length"]:
                    ...
                    out_of_char = f'[^{self.opt["dataset"]["characters"]}]'
    ...
    def __getitem__(self, index):
        ...
        with self.env.begin(write=False) as txn:
            ...
            try:
                ...
                if self.opt["dataset"]["img_color"]:
                    img = Image.new('RGB', (self.opt["dataset"]["imgW"], self.opt["dataset"]["imgH"]))
                else:
                    img = Image.new('L', (self.opt["dataset"]["imgW"], self.opt["dataset"]["imgH"]))
                label = '[dummy_label]'

            if not self.opt["dataset"]["sensitive_mode"]:
                label = label.lower()
